...
function getPublicIp() {
    fetch("https://geolocation-db.com/json/", {
      ...
      .then((data) => {
        currentCity = data.city || "Mauritius"; // Use Mauritius as fallback
        getWeatherData(currentCity, currentUnit, hourlyorWeek);
      })
      .catch((err) => {
        console.error(err);
        // Fallback to default city
        currentCity = "Mauritius";
        getWeatherData(currentCity, currentUnit, hourlyorWeek);
      });
  }
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        fetch(
          `https://api.opencagedata.com/geocode/v1/json?q=${lat}+${long}&key=bd408a17c91741b4858cfe1705a64d88`
        )
          .then((response) => response.json())
          .then((data) => {
            if (data.results && data.results.length > 0) {
              currentCity = data.results[0].components.city || "Mauritius";
            } else {
              currentCity = "Mauritius"; // Fallback
            }
            getWeatherData(currentCity, currentUnit, hourlyorWeek);
          })
          .catch((err) => {
